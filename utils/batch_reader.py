...
class Batcher(object):
    ...
    def __init__(self, data_path, vocab, hps,
        ...
        for _ in xrange(4):
            self._bucketing_threads.append(
                Thread(target=self._FillBucketInputQueue))
    ...
    def _FillInputQueue(self):
        ...
        while True:
            ...
            for i in xrange(min(self._max_article_sentences,
                ...
                enc_inputs += data.GetWordIds(
                    article_sentences[i], self._vocab)
            for i in xrange(min(self._max_abstract_sentences,
                                len(abstract_sentences))):
                dec_inputs += data.GetWordIds(
                    abstract_sentences[i], self._vocab)
