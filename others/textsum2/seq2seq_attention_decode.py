...
class BSDecoder(object):
  ...
  def _Decode(self, saver, sess):
    ...
    if not (ckpt_state and ckpt_state.model_checkpoint_path):
      tf.compat.v1.logging.info('No model to decode yet at %s', FLAGS.log_root)
      return False

    tf.compat.v1.logging.info('checkpoint path %s', ckpt_state.model_checkpoint_path)
    ckpt_path = os.path.join(
        FLAGS.log_root, os.path.basename(ckpt_state.model_checkpoint_path))
    tf.compat.v1.logging.info('renamed checkpoint path %s', ckpt_path)
  ...
  def _DecodeBatch(self, article, abstract, output_ids):
    ...
    tf.compat.v1.logging.info('abstract: %s', abstract)
    tf.compat.v1.logging.info('decoded:  %s', decoded_output)
