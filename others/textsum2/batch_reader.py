  ...
  def _FillInputQueue(self):
    ...
    while True:
      ...
      if (len(enc_inputs) < self._hps.min_input_len or
        ...
        tf.compat.v1.logging.warning('Drop an example - too short.\nenc:%d\ndec:%d',
      ...
      if not self._truncate_input:
        if (len(enc_inputs) > self._hps.enc_timesteps or
          ...
          tf.compat.v1.logging.warning('Drop an example - too long.\nenc:%d\ndec:%d',
                             len(enc_inputs), len(dec_inputs))
          continue
    ...
    while True:
      ...
      for t in self._input_threads:
        if t.is_alive():
          ...
          tf.compat.v1.logging.error('Found input thread dead.')
      ...
      for t in self._bucketing_threads:
        if t.is_alive():
          ...
          tf.compat.v1.logging.error('Found bucketing thread dead.')
