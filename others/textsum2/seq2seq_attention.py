...
def _Train(model, data_batcher):
  ...
  with tf.device('/cpu:0'):
    ...
    sv = tf.compat.v1.train.Supervisor(logdir=FLAGS.log_root,
                             is_chief=True,
                             saver=saver,
                             summary_op=None,
    ...
    sess = sv.prepare_or_wait_for_session(config=tf.compat.v1.ConfigProto(
...
def _Eval(model, data_batcher, vocab=None):
  ...
  while True:
    ...
    if not (ckpt_state and ckpt_state.model_checkpoint_path):
      tf.compat.v1.logging.info('No model to eval yet at %s', FLAGS.train_dir)
      continue
