...
def _extract_argmax_and_embed(embedding, output_projection=None,
  ...
  def loop_function(prev, _):
    ...
    if output_projection is not None:
      prev = tf.compat.v1.nn.xw_plus_b(
          prev, output_projection[0], output_projection[1])
    prev_symbol = tf.argmax(input=prev, axis=1)
    # Note that gradients will not propagate through the second parameter of
    # embedding_lookup.
    emb_prev = tf.nn.embedding_lookup(params=embedding, ids=prev_symbol)
...
class Seq2SeqAttentionModel(object):
  ...
  def _add_placeholders(self):
    ...
    hps = self._hps
    self._articles = tf.compat.v1.placeholder(tf.int32,
                                    [hps.batch_size, hps.enc_timesteps],
                                    name='articles')
    self._abstracts = tf.compat.v1.placeholder(tf.int32,
                                     [hps.batch_size, hps.dec_timesteps],
                                     name='abstracts')
    self._targets = tf.compat.v1.placeholder(tf.int32,
                                   [hps.batch_size, hps.dec_timesteps],
                                   name='targets')
    self._article_lens = tf.compat.v1.placeholder(tf.int32, [hps.batch_size],
                                        name='article_lens')
    self._abstract_lens = tf.compat.v1.placeholder(tf.int32, [hps.batch_size],
                                         name='abstract_lens')
    self._loss_weights = tf.compat.v1.placeholder(tf.float32,
                                        [hps.batch_size, hps.dec_timesteps],
                                        name='loss_weights')
  ...
  def _add_seq2seq(self):
    ...
    with tf.compat.v1.variable_scope('seq2seq'):
      encoder_inputs = tf.unstack(tf.transpose(a=self._articles))
      decoder_inputs = tf.unstack(tf.transpose(a=self._abstracts))
      targets = tf.unstack(tf.transpose(a=self._targets))
      loss_weights = tf.unstack(tf.transpose(a=self._loss_weights))
      article_lens = self._article_lens

      # Embedding shared by the input and outputs.
      with tf.compat.v1.variable_scope('embedding'), tf.device('/cpu:0'):
        embedding = tf.compat.v1.get_variable(
            'embedding', [vsize, hps.emb_dim], dtype=tf.float32,
            initializer=tf.compat.v1.truncated_normal_initializer(stddev=1e-4))
        emb_encoder_inputs = [tf.nn.embedding_lookup(params=embedding, ids=x)
                              for x in encoder_inputs]
        emb_decoder_inputs = [tf.nn.embedding_lookup(params=embedding, ids=x)
                              for x in decoder_inputs]

      for layer_i in xrange(hps.enc_layers):
        with tf.compat.v1.variable_scope('encoder%d'%layer_i), tf.device(
            self._next_device()):
          cell_fw = tf.compat.v1.nn.rnn_cell.LSTMCell(
              hps.num_hidden,
              initializer=tf.compat.v1.random_uniform_initializer(-0.1, 0.1, seed=123),
              state_is_tuple=False)
          cell_bw = tf.compat.v1.nn.rnn_cell.LSTMCell(
              hps.num_hidden,
              initializer=tf.compat.v1.random_uniform_initializer(-0.1, 0.1, seed=113),
              state_is_tuple=False)
          (emb_encoder_inputs, fw_state, _) = tf.compat.v1.nn.static_bidirectional_rnn(
      ...
      with tf.compat.v1.variable_scope('decoder'), tf.device(self._next_device()):
      ...
      with tf.compat.v1.variable_scope('output'), tf.device(self._next_device()):
        model_outputs = []
        for i in xrange(len(decoder_outputs)):
          if i > 0:
            tf.compat.v1.get_variable_scope().reuse_variables()
          model_outputs.append(
              tf.compat.v1.nn.xw_plus_b(decoder_outputs[i], w, v))

      if hps.mode == 'decode':
        with tf.compat.v1.variable_scope('decode_output'), tf.device('/cpu:0'):
          best_outputs = [tf.argmax(input=x, axis=1) for x in model_outputs]
          tf.compat.v1.logging.info('best_outputs%s', best_outputs[0].get_shape())
          self._outputs = tf.concat(
              axis=1, values=[tf.reshape(x, [hps.batch_size, 1]) for x in best_outputs])

          self._topk_log_probs, self._topk_ids = tf.nn.top_k(
              tf.math.log(tf.nn.softmax(model_outputs[-1])), hps.batch_size*2)

      with tf.compat.v1.variable_scope('loss'), tf.device(self._next_device()):
        ...
        tf.compat.v1.summary.scalar('loss', tf.minimum(12.0, self._loss))
  ...
  def _add_train_op(self):
    ...
    self._lr_rate = tf.maximum(
        ...
        tf.compat.v1.train.exponential_decay(hps.lr, self.global_step, 30000, 0.98))

    tvars = tf.compat.v1.trainable_variables()
    with tf.device(self._get_gpu(self._num_gpus-1)):
      grads, global_norm = tf.clip_by_global_norm(
          tf.gradients(ys=self._loss, xs=tvars), hps.max_grad_norm)
    tf.compat.v1.summary.scalar('global_norm', global_norm)
    optimizer = tf.compat.v1.train.GradientDescentOptimizer(self._lr_rate)
    tf.compat.v1.summary.scalar('learning rate', self._lr_rate)
