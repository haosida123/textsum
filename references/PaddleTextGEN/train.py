...
def main():
    ...
    train_data, valid_data = (x_train, x_val), (y_train, y_val)
    test_data = None
    ...
    def train():
        ...
        print('start total {} epoch(s), {} steps per epoch, batch size {}\n'.
              format(max_epoch, steps_per_epoch, batch_size))
        for epoch_id in range(max_epoch):
            ...
            for batch_id, batch in enumerate(train_data_iter):
                ...
                print("\r-- Epoch %d/%d; Batch %d/%d; Time %.0f, ETA %.0f s; ppl %.5f" %
                      (epoch_id, max_epoch, batch_id, steps_per_epoch,
                       time.time() - batch_start_time,
                       (time.time() - batch_start_time) / (batch + 1) *
                       (steps_per_epoch - batch),
                       np.exp(total_loss / word_count)), end='')
            ...
            if not args.profile:
                ...
                print("save finished")
                dev_ppl = eval_(valid_data)
                print("dev ppl", dev_ppl)
                test_ppl = eval_(test_data) if test_data is not None else None
