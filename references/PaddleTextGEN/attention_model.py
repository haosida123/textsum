...
class AttentionDecoderCell(DecoderCell):
    ...
    def call(self, step_input, states, enc_output, enc_padding_mask=None):
        ...
        for i in range(self.num_layers):
            out, new_lstm_state = self.lstm_cells[i](
                step_input, lstm_states[i])
...
class AttentionModel(BaseModel):
    def __init__(self, *args, **kwargs):
        ...
        super(AttentionModel, self).__init__(*args, **kwargs)
        # hidden_size,
        # src_vocab_size,
        # tar_vocab_size,
        # batch_size,
    ...
    def _build_decoder(self, enc_final_state, mode='train', beam_size=10):
        if self.embedding_matrix is None:
            ...
            def output_layer(x):
                layer1 = lambda xx: layers.fc(
                    ...
                    bias_attr=False)
                layer0 = lambda xx: layers.fc(
                    xx,
                    size=self.embedding_matrix.shape[1],
                    num_flatten_dims=len(x.shape) - 1,
                    param_attr=fluid.ParamAttr(
                        name="output_w0",
                        initializer=fluid.initializer.UniformInitializer(
        ...
        if mode == 'train':
            ...
            def output_layer(x): return layers.fc(x,
                                                  size=self.tar_vocab_size,
                                                  num_flatten_dims=len(
                                                      x.shape) - 1,
                                                  param_attr=fluid.ParamAttr(
                                                      name="output_w"),
                                                  bias_attr=False)
