...
uniform_initializer = lambda x: None  # None -> Xavier
...
class BaseModel(object):
    def __init__(self,
        ...
        if embedding_matrix is None:
            param_attr_fn = lambda name: fluid.ParamAttr(
                name=name, initializer=uniform_initializer(init_scale))
        else:
            param_attr_fn = lambda name: fluid.ParamAttr(
                name=name,
        ...
        self.src_embeder = lambda x: fluid.embedding(
            ...
            dtype='float32',
            is_sparse=False,
            param_attr=param_attr_fn('source_embedding'))
        self.tar_embeder = lambda x: fluid.embedding(
            input=x,
            size=[self.tar_vocab_size, self.hidden_size],
            dtype='float32',
            is_sparse=False,
            param_attr=param_attr_fn('target_embedding'))
