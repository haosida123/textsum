...
class Seq2seq_attention():
    ...
    def loss_function(self, real, pred, sequence_length):
        mask = tf.math.logical_not(tf.math.equal(real, 0))
        loss_ = self.loss_object(real, pred)
        mask = tf.cast(mask, dtype=loss_.dtype)
        loss_ *= mask
        loss_ *= 1.0 / tf.math.exp(sequence_length / 100.0)
    ...
    def train_step(self, inp, targ, enc_hidden, begin_id, batch_size,
        ...
        sequence_length = tf.reduce_sum(
            tf.cast(tf.math.logical_not(tf.math.equal(targ, 0)),
                    tf.float32), axis=1)
        with tf.GradientTape() as tape:
            ...
            for t in tf.range(1, y_seq_len):
                ...
                loss += self.loss_function(
                    targ[:, t], predictions, sequence_length)
                # Coverage loss
                ...
                loss += attention_coverage_loss * self.params.coverage_weight
    ...
    def train_epoch(
        ...
        if restore_checkpoint:
            ...
            try:
                ...
                if input('continue without restoring?(y/n)').lower() != 'y':
                    return
        print('start total {} epoch(s), {} steps per epoch, batch size {}\n'.format(
        ...
        for epoch in range(epochs):
            ...
            for (batch, (inp, targ)) in enumerate(dataset.take(steps_per_epoch)):
                ...
                print('\rEpoch {}/{}  Batch {}/{}  Loss {:.4f}  Time {:.0f}s  Eta {:.0f}s'.
                      format(epoch + 1, epochs, batch, steps_per_epoch,
                             batch_loss.numpy(), time.time() - start,
                             (time.time() - start) / (batch + 1) *
                             (steps_per_epoch - batch)), end="")
